...
public class WeaponPrefabGenerator : EditorWindow
{
    ...
    new WeaponData {name="M200",weaponType=WeaponType.rifle,treeDamage=0f,oreDamage=0f,headDamage=120f,bodyDamage=95f,legDamage=70f,isThrowable=false,hasAreaEffect=false,fireRate=45, magSize=5,pelletCount=1,weaponPrefab_rb="rb_M200",weaponPrefab_onplayer="onplayer_M200"},
    new WeaponData {name="M24",weaponType=WeaponType.rifle,treeDamage=0f,oreDamage=0f,headDamage=110f,bodyDamage=85f,legDamage=60f,isThrowable=false,hasAreaEffect=false,fireRate=50,magSize=5,pelletCount=1,weaponPrefab_rb="rb_M24",weaponPrefab_onplayer="onplayer_M24"}
};
    ...
    {
        GetWindow<WeaponPrefabGenerator>("Weapon Prefab Generator");
    }
...
{
    // Basic Info
    public string name;                     // Weapon display name
    public WeaponType weaponType;           // melee, sidearm, rifle

    // Damage
    public float treeDamage;                // For cutting trees
    public float oreDamage;                 // For mining ores
    public float headDamage;                // Damage to head
    public float bodyDamage;                // Damage to body
    public float legDamage;                 // Damage to legs
    ...
    public Sprite scopeTexture;              // Optional: scope sprite for ranged weapons
    public float scopeZoom = 1f;             // Optional: zoom level for scoped weapons

    // Flags
    public bool isThrowable = false;        // Can the weapon be thrown
    public bool hasAreaEffect = false;      // Explosives or AoE weapons
    public bool isAutomatic = true;         // Automatic fire? (if false, semi-auto)
    public bool isStackable = false;        // Can this be stacked in inventory

    // Prefabs
    public string weaponPrefab_rb;          // In-world rigidbody prefab
    public string weaponPrefab_onplayer;    // Player-held prefab

    // Parented Positions (optional)
    public Vector3 position;                // Held position
    public Vector3 rotation;                // Held rotation
    public Vector3 scale = Vector3.one;     // Held scale

    public Vector3 storedPosition;          // Stored on player (inventory) position
    ...
    public WeaponData(string name, WeaponType type)
    {
        this.name = name;
