...
public partial class GameManager : NetworkBehaviour, IVfxService
{

    [Header("Visual Effects Manager\n------------------------------")]
    ...
    public void RequestVfxServerByName(string vfxName, Vector3 position, Vector3 rotation)
    {
    ...
    public void PlayVfxLocalByName(string vfxName, Vector3 position, Vector3 rotation)
    {
        if (_nameToIdLookup.TryGetValue(vfxName, out string id))
        {
    ...
    public void RequestVfxServer(string vfxId, Vector3 position, Vector3 rotation)
    {
        PlayVfx_ServerRpc(vfxId, position, rotation);
    }

    [ServerRpc(RequireOwnership = false)]
    private void PlayVfx_ServerRpc(string vfxId, Vector3 position, Vector3 rotation)
    {
        if (!IsServer) return;
    ...
    [ClientRpc]
    private void PlayVfx_ClientRpc(string vfxId, Vector3 position, Vector3 rotation)
    {
        PlayVfxLocal(vfxId, position, rotation);
    }

    public void PlayVfxLocal(string vfxId, Vector3 position, Vector3 rotation)
    {
        ...
        if (pool.Count > 0)
        {
            ...
            effect.transform.localRotation = Quaternion.LookRotation(rotation);
